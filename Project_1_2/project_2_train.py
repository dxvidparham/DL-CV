...
SIZE = config.size
SIZE_PROP_BATCH = config.size_prop_batch
...
def generate_targets(prop, gt):
    targets = []
    for bb in prop:
        targets.append(torch.tensor([IoU(bb,obj) for obj in gt]).argmax(0))
...
def crop_and_resize(proposals, size):
        # crop & resize
        img_list = []
...
for epoch in tqdm(range(num_epochs), unit='epoch'):
    ...
    for minibatch_no, (imgs, annotations) in tqdm(enumerate(trainloader), total=len(trainloader)):
        imgs = list(img.to(device) for img in imgs)
        annotations = [{k: v.to(device) for k, v in t.items()} for t in annotations]
        for img, annotation in tqdm(enumerate(zip(imgs,annotations)), unit="image", total=len(imgs)):

            # get the proposal frames for the image_id
            proposals = get_proposals(annotations["path"])
            # crop and resize
            img_list = crop_and_resize(proposals, size=SIZE_PROP_BATCH)
    ...
    with torch.no_grad():
        ...
        for imgs, annotations in testloader:
            ...
            for proposals_batch in chunker(proposals_torch, SIZE_PROP_BATCH):
            
                batch_output = model(proposals_batch)

                output.append(batch_output)

            target = generate_targets(torch.tensor(proposals), annotations["boxes"])
        ...
        train_acc = train_correct/len(proposals)
        test_acc = test_correct/len(proposals)
    nms = NMS()
    mAP = compute_mAP()
