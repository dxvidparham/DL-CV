...
class ISICDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        for path in self.image_paths:
            ...
            self.mask_paths.append(sorted(glob.glob(f"{data_path}/Segmentations/{file_name}*.png"))[0])
        if len(self.mask_paths) == len(self.image_paths): print("dataset size correct")
