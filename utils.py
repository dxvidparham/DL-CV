...
def visualize_results(images, predicted, label):
    ...
    for k, (img, pred, gt) in enumerate(zip(images, predicted, label)):
        ...
        pred = pred.permute(1, 2, 0).clamp(0, 255).to(torch.uint8)
        gt = gt.permute(1, 2, 0).clamp(0, 255).to(torch.uint8)
        subplots[k].imshow(img.cpu().numpy())
        subplots[k].imshow(pred.cpu().numpy(), "hot", alpha=0.3)
        subplots[k].imshow(pred.cpu().numpy(), "jet", alpha=0.3)
        subplots[k].axis('off')
